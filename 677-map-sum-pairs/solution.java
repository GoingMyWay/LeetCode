class MapSum {
    ...
    public int sum(String prefix) {
        List<Integer> res = trie.ValuesWithPrefix(prefix);
        int sum = 0;
        for (int v: res) sum += v;
        return sum;
    }
 ...
 */
 ...
 class Trie {
    ...
    public List<Integer> ValuesWithPrefix(String word) {
        return map.ValuesWithPrefix(word);
    }
}
...
class TrieMap<V> {
    ...
    // 搜索前缀为 prefix 的所有键对应的值
    public List<Integer> ValuesWithPrefix(String prefix) {
        ...
        traverse2(x, new StringBuilder(prefix), res);
        return res;
    }

    // 遍历以 node 节点为根的 Trie 树，找到所有键
    private void traverse2(TrieNode<V> node, StringBuilder path, List<Integer> res) {
        if (node == null) {
            // 到达 Trie 树底部叶子结点
            return;
        }
        ...
        for (char c = 0; c < R; c++) {
            // 做选择
            path.append(c);
            traverse2(node.children[c], path, res);
            // 撤销选择
