...
private:
    bool valid(const string& s, int start, int length) {
        return length == 1 || (s[start] != '0' && stoi(s.substr(start, length)) <= 255);
    }

    void backtrack(const string& s, int startIndex, vector<int>& dots, vector<string>& ans) {
